 ...
 * @date 2017-07-18
 * @author DanRui
 * @see 定义高拍仪操作函数，包括拍照、上传、识别身份证等定义。
 */
...
function OpenVideoMain() {
	...
	VideoMain = plugin()
			.Device_CreateVideo(DeviceMain, nResolution, SelectType);
	if (VideoMain) {
...
function changesubTypeMain() {
	if (DeviceMain) {
		...
		var nResolution = plugin().Device_GetResolutionCountEx(DeviceMain,
				SelectType);
		sResolution.options.length = 0;
		for (var i = 0; i < nResolution; i++) {
			var width = plugin().Device_GetResolutionWidthEx(DeviceMain,
					SelectType, i);
			var heigth = plugin().Device_GetResolutionHeightEx(DeviceMain,
					SelectType, i);
			sResolution.add(new Option(width.toString() + "*"
					+ heigth.toString()));
		}
			...
			function(type, idx, dbt) {
				if (1 == type)//视频设备
				{
					if (1 == dbt)//设备到达
					{
						...
						{
							if (null == DeviceMain) {
								DeviceMain = plugin().Global_CreateDevice(1,
										idx);
								/*if (DeviceMain) {
									document.getElementById('lab1').innerHTML = plugin()
											.Device_GetFriendlyName(DeviceMain);

									var sSubType = document
											.getElementById('subType1');
									sSubType.options.length = 0;
									var subType = plugin().Device_GetSubtype(
											DeviceMain);
									if (subType & 1) {
...
function EnableDate(obj) {
	if (obj.checked) {
		...
		if (VideoMain) {
			...
			plugin().Video_EnableDate(VideoMain, font, width - offsetx,
		...
		if (VideoAssist) {
			...
			plugin().Video_EnableDate(VideoAssist, font, width - offsetx,
					heigth - offsety, 0xffffff, 0);
		}
...
function AddText(obj) {
	if (obj.checked) {
		...
		if (VideoMain) {
			plugin().Video_EnableAddText(VideoMain, font, 0, 0, "文字水印", 65280,
					150);
		}
		if (VideoAssist) {
			plugin().Video_EnableAddText(VideoAssist, font, 0, 0, "文字水印",
					65280, 150);
		}
		plugin().Font_Release(font);
	} else {
...
function changescansize() {
	...
	if (size == 0) {
		...
		rect = plugin().Global_CreateRect(width / 2 - width / 4,
				heigth / 2 - heigth / 4, width / 2, heigth / 2);
		MainView().View_SetState(2);//小尺寸
		MainView().View_SetSelectedRect(rect);

	} else if (size == 2) {
		rect = plugin().Global_CreateRect(width / 2 - width / 6,
				heigth / 2 - heigth / 6, width / 3, heigth / 3);
...
function UploadToHttpServer() {
	...
	var http = plugin().Global_CreateHttp(
			"http://localhost:8080/FileStreamDemo/servlet/FileSteamUpload?");//java服务器demo地址
	if (http) {
...
function UploadThumbToServer() {
	var http = thumb1().Thumbnail_HttpUploadCheckImage(
			"http://localhost:8080/FileStreamDemo/servlet/FileSteamUpload?", 0);
	if (http) {
...
function ScanToHttpServer() {
	if (VideoMain) {
		var img = plugin().Video_CreateImage(VideoMain, 0,
				MainView().View_GetObject());
		if (img) {
...
function StartMainRecord() {
	if (VideoMain) {
		...
		//设置水印
		plugin().VideoCap_SetWatermark(videoCapMain, 1, 0, 0, 4, 100, 0,
				"深圳新良田科技", RGB(255, 23, 140), "Microsoft YaHei", 72, 50, 0);
		...
		if (!plugin().VideoCap_PreCap(videoCapMain, "C:\\eloamPlugin_main.mp4",
				selMicIdx, FrameRate, 1, videoOutputWidth, videoOutputHeight)) {
			alert("录像设置失败");
			return;
		}
...
function StartAssistRecord() {
	if (VideoAssist) {
		...
		//设置水印
		plugin().VideoCap_SetWatermark(videoCapAssist, 1, 0, 0, 1, 220, 0,
				"深圳新良田科技", RGB(255, 23, 140), "Microsoft YaHei", 72, 50, 0);
		...
		if (!plugin().VideoCap_PreCap(videoCapAssist,
				"C:\\eloamPlugin_assist.mp4", selMicIdx, FrameRate, 1,
					...
					if (image) {
						...
						if (b) {
							if (plugin().Global_GetBarcodeCount() <= 0) {
								...
								for (var i = 0; i < plugin()
										.Global_GetBarcodeCount(); i++)
...
function VerifyFaceDetect() {
	if (VideoAssist) {
		var image = plugin().Video_CreateImage(VideoAssist, 0,
				AssistView().View_GetObject());
		var idcardImage = plugin().Global_CreateImageFromFile("C:\\idcard.jpg",
				0);
